...
- Fast turnaround and low latency.
- Structured itinerary generation.

**FastAPI Integration:**
FastAPI acts as the HTTP interface between Genkit (frontend) and ADK (backend). The ADK multi-agent pipeline generates components, and FastAPI aggregates them into structured JSON.

### 2.4 Platform & Services (Firebase)
- **Firebase AI Studio** — Integrated development environment with AI-assisted coding and deployments.
...
1. **Listen**: System receives unstructured input (e.g., "3-day cultural trip to Odisha").
2. **Route**: Frontend identifies user intent (e.g., "plan trip" vs "translate").
3. **Execute**: 
    - **Frontend**: Performs quick tasks (Q&A, translations).
    - **Backend**: ADK orchestrates specialized sub-agents to generate a detailed plan via Gemini 2.5 Flash.
...
WanderWise AI demonstrates the power of a dual-agent architecture combining Genkit and ADK. This hybrid design enables rich reasoning while keeping the application scalable and efficient.

**Future Enhancements:**
- Real-time tools (flight/weather/Maps APIs).
- Multi-modal planning using images.
- User personalization profiles in Firestore.
