...
export function SignInModal({ isOpen, onOpenChange }: { isOpen: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xs rounded-2xl bg-white/80 backdrop-blur-sm dark:bg-black/80 border-none">
        <DialogHeader className="items-center text-center">
            <WanderwiseLogo className="h-7 w-auto mb-1"/>
            <DialogTitle className="text-xl font-bold">Welcome to WanderWise</DialogTitle>
            <DialogDescription className="text-xs">Sign in or create an account to get started.</DialogDescription>
        </DialogHeader>
          ...
          <TabsContent value="sign-in">
            <form onSubmit={signInForm.handleSubmit(handleSignIn)} className="space-y-3">
              <div className="space-y-1">
                ...
                <Input id="email-in" type="email" placeholder="m@example.com" {...signInForm.register('email')} className="h-9 text-xs" />
              ...
              <div className="space-y-1">
                ...
                <Input id="password-in" type="password" {...signInForm.register('password')} className="h-9 text-xs" />
               ...
               <DialogFooter className="flex-col space-y-2 !mt-4">
                ...
                <Button type="submit" disabled={isLoading} className="w-full h-9 text-xs">
          ...
          <TabsContent value="sign-up">
            <form onSubmit={signUpForm.handleSubmit(handleSignUp)} className="space-y-3">
              <div className="space-y-1">
                ...
                <Input id="email-up" type="email" placeholder="m@example.com" {...signUpForm.register('email')} className="h-9 text-xs" />
              ...
              <div className="space-y-1">
                ...
                <Input id="password-up" type="password" {...signUpForm.register('password')} className="h-9 text-xs" />
              ...
              <div className="space-y-1">
                ...
                <Input id="confirmPassword-up" type="password" {...signUpForm.register('confirmPassword')} className="h-9 text-xs" />
        ...
        <div className="relative my-2">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-background/0 px-2 text-muted-foreground text-[10px]">Or continue with</span>
          </div>
        </div>

        <Button variant="outline" className="w-full rounded-full shadow-sm h-9 text-xs" onClick={handleGoogleSignIn} disabled={isLoading}>
