    ...
    <section className="container py-10 sm:py-14 scroll-mt-20" id="itinerary">
      <div className="flex items-center justify-between gap-4 mb-4">
        ...
        <div className="hidden sm:flex items-center gap-2 text-sm">
          <span className="px-2 py-1 rounded bg-secondary/80">On track ✅</span>
      ...
      <div className="grid gap-6">
        {sample.map((d) => (
          <article key={d.day} className="relative rounded-2xl border border-border bg-card/60 overflow-hidden shadow-sm animate-slide-in">
            <div className="absolute left-4 top-4 size-3 rounded-full bg-accent/70 animate-glow" />
            <div className="grid md:grid-cols-2">
              <div className="p-4 sm:p-6">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold">Day {d.day}: {d.destination}</h3>
                  {d.hiddenGem && (
                    <span className="text-xs px-2 py-1 rounded-full bg-accent/20 border border-accent/50">Hidden Gem ✨</span>
                  )}
                </div>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-3">
        <button className="rounded-md border border-border bg-background px-3 py-2 text-sm hover:bg-secondary/60 transition">Hospitals Nearby</button>
        <button className="rounded-md border border-border bg-background px-3 py-2 text-sm hover:bg-secondary/60 transition">Embassies</button>
        <button className="rounded-md border border-border bg-background px-3 py-2 text-sm hover:bg-secondary/60 transition">Police Stations</button>
      </div>
    </section>
  );
}
